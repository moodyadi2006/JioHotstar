...
export default function Home() {
  ...
  useEffect(() => {
    if (trendingTvShowsList.length > 0) {
      const interval = setInterval(() => {
        setCurrentIndex(
          (prevIndex) => (prevIndex + 1) % trendingTvShowsList.length
        );
  ...
  return (
    <div className="bg-black w-100">
          ...
          >
            <CarouselContent className="flex">
              {tvShowsAiringTodayList.map((movie, index) => (
                ...
                >
                  <Link href={`/movies/${movie.title || movie.name}`} passHref>
      ...
      <main className="ml-6 pb-8 group">
        <h1 className="text-2xl font-bold mb-4 text-white">Popular TV Shows</h1>
        <div className="relative">
          <Carousel
            className="relative"
            onSelect={(index) => setCurrentIndex(index)}
